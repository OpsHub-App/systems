# Cloud to Cloud Migration for Jira

**Migrate to Jira from Jira Cloud with zero downtime and complete data fidelity.**

[![Start Now](https://img.shields.io/badge/Start_Now-0052CC?style=for-the-badge&logo=atlassian&logoColor=white)](https://marketplace.atlassian.com/apps/2961517783)
...
OpsHub Migration Manager connects to Jira Cloud and Jira through their APIs and runs the migration in the background while your teams continue working.

The migration process follows three steps:

1. **Export** — OpsHub reads data from Jira Cloud via its API. No plugins or admin access required in the source system.
...
No. OpsHub runs migrations with zero downtime. Your teams continue working in Jira Cloud throughout the migration process. Incremental delta syncs capture any changes made after the initial load, so nothing is lost during the transition.
