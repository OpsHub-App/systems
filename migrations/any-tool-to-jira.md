# OpsHub Migration Manager (OMM) for Jira

**Migrate to Jira from Any Tool with zero downtime and complete data fidelity.**
...
OpsHub Migration Manager connects to Any Tool and Jira through their APIs and runs the migration in the background while your teams continue working.

The migration process follows three steps:

1. **Export** — OpsHub reads data from Any Tool via its API. No plugins or admin access required in the source system.
...
- Consolidating multiple legacy tools onto Jira
...
No. OpsHub runs migrations with zero downtime. Your teams continue working in Any Tool throughout the migration process. Incremental delta syncs capture any changes made after the initial load, so nothing is lost during the transition.
