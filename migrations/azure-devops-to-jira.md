...
OpsHub Migration Manager connects to Azure DevOps / TFS / VSTS and Jira through their APIs and runs the migration in the background while your teams continue working.

The migration process follows three steps:

1. **Export** — OpsHub reads data from Azure DevOps / TFS / VSTS via its API. No plugins or admin access required in the source system.
...
- Organizations moving from Azure DevOps to Jira for project management
...
No. OpsHub runs migrations with zero downtime. Your teams continue working in Azure DevOps / TFS / VSTS throughout the migration process. Incremental delta syncs capture any changes made after the initial load, so nothing is lost during the transition.
...
> **Need ongoing sync after migration?** See [Azure DevOps Integration for Jira](../integrations/azure-devops-jira.md) for real-time, bidirectional synchronization between Azure DevOps / TFS / VSTS and Jira.
