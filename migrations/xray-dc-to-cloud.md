# Xray on Jira DC to Xray on Cloud Migration

**Migrate to Jira from Xray (Data Center) with zero downtime and complete data fidelity.**

[![Start Now](https://img.shields.io/badge/Start_Now-0052CC?style=for-the-badge&logo=atlassian&logoColor=white)](https://marketplace.atlassian.com/apps/1535785919)

Migrate Xray test management data from Jira Data Center to Jira Cloud with zero downtime. Preserve all test cases, executions, and traceability during the transition.
...
Test cases, test plans, test executions, test sets, preconditions, test runs, and traceability links

All data is validated before, during, and after migration to ensure complete accuracy.

## How It Works

OpsHub Migration Manager connects to Xray (Data Center) and Jira through their APIs and runs the migration in the background while your teams continue working.

The migration process follows three steps:

1. **Export** — OpsHub reads data from Xray (Data Center) via its API. No plugins or admin access required in the source system.
...
- Moving Xray test data as part of a Jira DC to Cloud migration
- Preserving test management history during cloud transitions
- Zero-downtime migration of test management infrastructure
...
No. OpsHub runs migrations with zero downtime. Your teams continue working in Xray (Data Center) throughout the migration process. Incremental delta syncs capture any changes made after the initial load, so nothing is lost during the transition.
