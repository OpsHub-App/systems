...
OpsHub Migration Manager connects to Any ITSM Tool and Jira through their APIs and runs the migration in the background while your teams continue working.

The migration process follows three steps:

1. **Export** — OpsHub reads data from Any ITSM Tool via its API. No plugins or admin access required in the source system.
...
- Migrating from any ITSM tool to Jira Service Management
...
No. OpsHub runs migrations with zero downtime. Your teams continue working in Any ITSM Tool throughout the migration process. Incremental delta syncs capture any changes made after the initial load, so nothing is lost during the transition.
...
> **Need ongoing sync after migration?** See [OIM for Bidirectional JSM and Jira Integration](../integrations/jsm-jira.md) for real-time, bidirectional synchronization between Any ITSM Tool and Jira.
