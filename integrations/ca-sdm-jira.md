# CA Service Desk Manager (SDM) Integration for Jira

**Seamlessly connect CA Service Desk Manager and Jira with real-time, bidirectional sync.**

[![Get It Now](https://img.shields.io/badge/Get_It_Now-0052CC?style=for-the-badge&logo=atlassian&logoColor=white)](https://marketplace.atlassian.com/apps/1059270378)

Bidirectional integration between Broadcom CA Service Desk Manager and Jira. Synchronize incidents, requests, and change orders across both ITSM platforms.

## Why Integrate CA Service Desk Manager with Jira?
...
| **Always Accurate** | Real-time synchronization ensures CA Service Desk Manager and Jira always show the same information. No stale data, no conflicts. |
...
Incidents, requests, change orders, problems, tasks, comments, attachments, and custom fields
...
OpsHub Integration Manager connects CA Service Desk Manager and Jira through their native APIs, operating as an external engine between both platforms. No plugins are installed inside your systems, so there is zero performance impact on either tool.

The integration process follows three steps:

1. **Connect** — Provide API credentials for CA Service Desk Manager and Jira. Standard service account permissions are sufficient.
...
- IT teams using CA SDM for service management while development uses Jira
- Automated escalation from CA SDM tickets to Jira engineering tasks
- Unified ITSM reporting across CA SDM and Jira
...
**Why do teams integrate CA Service Desk Manager with Jira?**

CA Service Desk Manager and Jira are often used by different teams within the same organization. Without integration, teams waste time on manual data entry, work with outdated information, and lose visibility across workflows. OpsHub keeps both systems synchronized automatically, so each team continues working in their preferred tool while staying aligned on priorities and progress.
...
If you use CA Service Desk Manager with Jira, you may also need:
