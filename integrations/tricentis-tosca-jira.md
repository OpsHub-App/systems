# Tricentis Tosca Integration for Jira

**Seamlessly connect Tricentis Tosca and Jira with real-time, bidirectional sync.**

[![Get It Now](https://img.shields.io/badge/Get_It_Now-0052CC?style=for-the-badge&logo=atlassian&logoColor=white)](https://marketplace.atlassian.com/apps/1237180)

Rich data sync between Tricentis Tosca and Jira for TestCases, ExecutionLogs, TCFolders, Requirements, Issues, and Modules. Enable test automation at scale with a no-code setup.

## Why Integrate Tricentis Tosca with Jira?
...
| **Always Accurate** | Real-time synchronization ensures Tricentis Tosca and Jira always show the same information. No stale data, no conflicts. |
...
Test cases, execution logs, test case folders, requirements, issues, modules, and results
...
OpsHub Integration Manager connects Tricentis Tosca and Jira through their native APIs, operating as an external engine between both platforms. No plugins are installed inside your systems, so there is zero performance impact on either tool.

The integration process follows three steps:

1. **Connect** — Provide API credentials for Tricentis Tosca and Jira. Standard service account permissions are sufficient.
...
- QA automation teams on Tosca while developers and PMs use Jira
- Linking automated test results back to Jira user stories and defects
- Enterprise test management with centralized visibility in Jira
...
**Why do teams integrate Tricentis Tosca with Jira?**

Tricentis Tosca and Jira are often used by different teams within the same organization. Without integration, teams waste time on manual data entry, work with outdated information, and lose visibility across workflows. OpsHub keeps both systems synchronized automatically, so each team continues working in their preferred tool while staying aligned on priorities and progress.
...
If you use Tricentis Tosca with Jira, you may also need:

- [Tricentis qTest and Jira Integration](tricentis-qtest-jira.md)
- [TestRail Integration for Jira](testRail-jira.md)
- [IBM Engineering Test Management (ETM) Integration for Jira](ibm-etm-jira.md)
